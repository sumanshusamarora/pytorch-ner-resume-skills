...
class EntityExtraction(nn.Module):
    ...
    def forward(self, x_word, x_char, x_pos):
        ...
        binary_out_repeat = binary_out.repeat_interleave(concat.shape[1], 0).view(batch_size, concat.shape[1], -1)
...
class ClassificationModelUtils:
    def __init__(self, dataloader_train, dataloader_test, binary_class_weights, ner_class_weights, cuda=True):
        if cuda:
        ...
        self.criterion_crossentropy = nn.NLLLoss(weight=torch.FloatTensor(self.ner_class_weights).to(device))
    ...
    def validate(self):
        ...
        for k, data_test in enumerate(self.dataloader_test):
            with torch.no_grad():
                ...
                test_accuracy, test_precision, test_recall, test_f1 = self.evaluate_classification_metrics(
    ...
    def train(self, num_epochs=10):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_num, data in enumerate(dataloader_train):
                ...
                accuracy, precision, recall, f1 = self.evaluate_classification_metrics(binary_batch_all, binary_out_result_all)
...
if __name__ == "__main__":
    ...
    with mlflow.start_run() as run:
        ...
        # Encode y binary
        y_binary_encode_map, y_binary_encoded_train, y_binary_encoded_test = encode_binary_y(y_binary_list_train, y_binary_list_test)
        ...
        #Build model
        model_utils = ClassificationModelUtils(dataloader_train, dataloader_test, binary_class_weights, ner_class_weights)
